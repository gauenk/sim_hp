...
Thus the metropolis-within-gibbs algorithm is as follows
...
\begin{algorithm}[h]
  ...
  $l_i | \{v_{k}\}_{k=1}^i \sim \text{Bern}(\frac{\lambda(v_i | S_i)}{\eta(v_i)})$,
  $S_i = \{v_j \in V|\; j < i, l_j = 1\}$\;
...
Our Gibbs sampler targets the state $(V,L,Z)$ or $(U,S,L,Z)$. We note that $U$ and $S$ contain all the information for $L$, but we write our state including $L$ for clarity of writing. The Gibbs sampler alternates between (i) simulating thinned events $U|S \sim \text{PoissonProcess}(\eta(t) - \lambda(t | S_t))$ where $S_t$ are all events in $S$ before time $t$, and (ii) simulating censored events $L|U,S,Z$ with either a Metroplis-within-Gibbs algorithm or Forward-Filtering Backward-Sampling (FFBS).
...
\begin{algorithm}[h]
  ...
  Simulate $Z|X,Y,U$ via metropolis-within-gibbs\;
  \algrule
...
\begin{algorithm}[h]
  \DontPrintSemicolon
  \SetAlgoLined
  \KwIn{$V$ virtual events ,$L$ thinning mask, $Z$ missing events}
  \KwOut{New Values for $L$ and $Z$}
  \algrule
  Augmented state space $Z \in \{-1,0,1\}^N$\;
  \For{$z_i \in Z$}{
    \If{$z = 0$ we move event from $Y$ to $U$}{
      with prob $\alpha_{y2u}$ $l_i 1 \to 0$ and $z_i 0 \to 1$.
    }\Elif{$z = -1$ we move event from $U$ to $Y$}{
      with prob $\alpha_{u2y}$ $l_i 1 \to 0$ and $z_i 0 \to 1$.
    }
  }
  \algrule
