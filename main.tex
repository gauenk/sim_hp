...
% include mechanism for virtual events (2)
% Y,V | X
% via Gibbs sampler
% simulate V | X, Y, Z
% [not really] simulate Z | X, Y, V NO
% simulate V' cup Y' | X, Y, V
% define z to also be to {x,y,v} not 0,1
% simulate Y | X, V, Z
% Y, Z, V | X
\noindent The generative model for a Censored Hawkes Process is given in Algorithm~\ref{alg:gen_censored_hp}. The original trajectory, $S$, is simulated from a Hawkes Process with parameters $\vtheta$. Eah sample is marked with a binary value $z_i \in \{0,1\}$ indicating if the resulting event is observed or censored (missing). Here, we assume all events are observed with probability $\alpha$. Using the binary values, we separate the trajectory into the observed set $X$ and the censored set $Y$.
...
\begin{algorithm}[H]
  ...
  $\widetilde{Y} = \{v_i : z_i = 1, z_i \in Z\}$
  \caption{Proposed 1-step algorithm for sampling censored data from a Hawkes Process}
...
\noindent And the probability the event remains virtual,
